...
func (c *Client) reconnect(ctx context.Context) error {
	...
	if err != nil {
		utils.LogError("RPC client reconnect failed", "err", err)
		return err
	}
	select {
...
func (c *Client) dispatch(codec ServerCodec) {
	...
	for {
		...
		case err := <-c.readErr:
			utils.LogDebug("RPC connection read error", "err", err)
			conn.close(err, lastOp)
			reading = false

		// Reconnect:
		case newcodec := <-c.reconnected:
			utils.LogDebug("RPC client reconnected", "reading", reading, "conn", newcodec.remoteAddr())
			if reading {
